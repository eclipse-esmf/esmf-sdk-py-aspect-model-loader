...
class PropertyInstantiator(InstantiatorBase[Property]):
    ...
    def _create_instance(self, element_node: Node) -> Property:
        """
        Instantiates a property by eagerly resolving all attributes (no lazy properties).
    ...
    def _create_property_direct_reference(self, element_node: URIRef) -> Property:
        """Constructs a DefaultProperty instance with all attributes eagerly resolved (no lazy properties)."""
    ...
    def _create_property_blank_node(self, element_node: BNode) -> Property:
        """The given node is a blank node holding a reference to the property and having additional attributes. All attributes are eagerly resolved."""
        ...
        not_in_payload_node = self._aspect_graph.value(subject=element_node, predicate=self._samm.get_urn(SAMM.not_in_payload))
    ...
    def _create_property_with_extends(self, element_node: BNode) -> Property:
        """Constructs a DefaultPropertyWithExtends instance with all attributes eagerly resolved (no lazy properties)."""
        payload_name = self._get_child(element_node, self._samm.get_urn(SAMM.payload_name))
